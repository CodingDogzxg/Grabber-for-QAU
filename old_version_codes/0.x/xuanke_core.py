#coding=utf-8
import PJ  # 不知道为啥评教的类会出现在这里
import os, re, \
from io import StringIO
import time, urllib
import requests
from PIL import Image  # 这是py2的模块 py3请使用Pillow
from xkocr import OCR  # 这个模块是自己写的 因为pytesser不好用


class Xuanke(PJ.PJ):  # 选课继承了评教类中的connection 可以拿出来 不必继承
	"""docstring for Xuanke"""
	def __init__(self):
		PJ.PJ.__init__(self)
		self.OCR_OBJ = OCR.Val_to_Str(os.path.split(os.path.realpath(__file__))[0]+"/dump-fuck.txt")
		self.Xuanke_Target_url = "http://222.30.49.10/xsxk/swichAction.do"  # 目标url
		self.Xuanke_Valcode_url = "http://222.30.49.10/SelectValidateCode"  # 验证码的url
		self.Xuanke_Inquire_Course_url = "http://222.30.32.3/apps/xksc/search.asp"  # 课程代码的url
	...
	def Xuanke(self, Array_of_Course):
		...
		Data = self.Session.post(self.Xuanke_Target_url, postdata,
	...
	def Get_Course_Name(self, Course_Code):
		...
		if Name:
			...
			try:
				Data = self.Inquire_Session.post(self.Xuanke_Inquire_Course_url, data = formdata,
												 headers ={'Content-Type': 'application/x-www-form-urlencoded'}
